...
export default function JobApplicantsPage() {
  ...
  const { data: applicantsData, isLoading: applicantsLoading } = useJobApplicantsQuery(id!);
        ...
        {row.answers.map((ans: { questionId: string; value: string }, idx: number) => {
          ...
          return (
            <div key={ans.questionId || idx} className="flex flex-col gap-0.5 text-sm">
              <span className="font-semibold text-slate-600 text-xs">{label}</span>
              <span className="text-slate-800">{ans.value || <em className="text-slate-400">No answer</em>}</span>
            </div>
          );
        })}
      </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          <nav className="flex items-center gap-2 text-sm font-medium text-slate-500">
            <Link to="/recruiter/dashboard" className="hover:text-blue-600 transition-colors flex items-center gap-1">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 mb-2 flex items-center gap-3">
                Applicant Pipeline
                {!isLoading && (
                  <span className="bg-blue-100 text-blue-700 text-sm py-1 px-3 rounded-full">{applicants.length} Total</span>
                )}
              </h1>
              <p className="text-slate-500 font-medium">
                {jobLoading ? "Loading job details..." : `Managing candidates for ${jobData?.title || "Unknown Job"}`}
              </p>
            </div>
            
            <Link to={`/jobs/${id}`}>
              <Button label="View Public Posting" icon="pi pi-external-link" outlined className="!border-slate-300 !text-slate-700 hover:!bg-slate-50 !rounded-xl px-6 py-2.5 font-bold" />
            </Link>
          </div>
        </div>
              ...
              <TabPanel header="Table View" leftIcon="pi pi-list mr-2">
                <div className="p-4">
                  <DataTable 
                    ...
                    rowHover
                  >
                    <Column field="applicantName" header="Name" sortable className="font-bold text-slate-900" style={{ minWidth: '150px' }}></Column>
                    <Column field="profileSummary" header="Profile Summary" body={(r) => <div className="text-sm text-slate-500 line-clamp-2 max-w-xs">{r.profileSummary || "No summary provided"}</div>}></Column>
                    <Column header="Screening Answers" body={answersBodyTemplate}></Column>
                    <Column field="resumeUrl" header="Resume" body={resumeBodyTemplate} align="center"></Column>
                    <Column header="Pipeline Stage" body={statusBodyTemplate} style={{ minWidth: '200px' }}></Column>
                  </DataTable>
                </div>
              </TabPanel>
              ...
              <TabPanel header="Kanban Board" leftIcon="pi pi-objects-column mr-2">
                <div className="p-4 m-4 flex flex-col gap-4">
                  ...
                  {[...STATUS_OPTIONS, { label: "Withdrawn", value: "Withdrawn", severity: "secondary" as const }].map(statusObj => {
                    ...
                    return (
                      <div key={statusObj.value} className={`rounded-2xl border p-5 ${
                        isWithdrawn
                        ...
                        {columnApplicants.length === 0 ? (
                          <div className="text-sm text-slate-400 italic pl-1">No candidates at this stage.</div>
                        ) : (
                          <div className="flex flex-wrap gap-4">
                            {columnApplicants.map(app => (
                              <div key={app.applicationId} className={`bg-white rounded-2xl shadow-sm border border-slate-200 p-5 flex flex-col gap-2 w-full sm:w-72 group ${
                                isWithdrawn ? "grayscale opacity-60" : "hover:shadow-md hover:border-blue-200 transition-all cursor-pointer"
                              }`}>
                                <div className={`font-bold ${
                                  isWithdrawn ? "text-slate-500 line-through" : "text-slate-900 group-hover:text-blue-600 transition-colors"
                                }`}>{app.applicantName}</div>
                                <div className="text-xs text-slate-500 line-clamp-2 leading-relaxed">
                                  {app.profileSummary || "No summary available."}
                                </div>
                                {app.resumeUrl && !isWithdrawn && (
                                  <a href={app.resumeUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-blue-600 text-xs font-semibold bg-blue-50 w-max px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors">
